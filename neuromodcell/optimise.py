...
class OptimiseModulation:
    ...
    def setup_load(self):
        ...
        self.modulation_setup["neuromodulation_dir"] = self.setup.parent / self.modulation_setup["neuromodulation_dir"]
        #print(self.modulation_setup["neuromodulation_dir"])
        print(f" printing {self.setup.parent}")
    ...
    def select(self):
        ...
        for trial, voltage in self.v_save.items():
            ...
            for i, select_method in enumerate(self.modulation_setup['selection_criteria']):
                ...
                opt_result = method(criteria=select_method["criteria"],
                                    voltages=[np.array(self.control_v), np.array(voltage)])
                trial_test[i] = opt_result['boolean']

            if np.all(trial_test):

                for receptor_description in self.unit_modulation["receptor"][trial]:
                    self.cell_model_pass_receptor[trial] = receptor_description
