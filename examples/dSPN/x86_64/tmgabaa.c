...
extern "C" {
 ...
 static double _hoc_modulationB();
 static double _hoc_modulationA();
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "modulationB", _hoc_modulationB,
 "modulationA", _hoc_modulationA,
 "urand", _hoc_urand,
 0, 0
};
#define modulationB modulationB_tmGabaA
#define modulationA modulationA_tmGabaA
...
/*VERBATIM*/
   ...
   if ( urand ( _threadargs_ ) > ( failRate * failRateB * modB * levelB ) ) {
...
static double _hoc_modulationA(void* _vptr) {
 double _r;
    _hoc_setdata(_vptr);
 _r =  modulationA (  );
 return(_r);
...
static double _hoc_modulationB(void* _vptr) {
 double _r;
    _hoc_setdata(_vptr);
 _r =  modulationB (  );
 return(_r);
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   g = ( B - A ) * modulationA ( _threadargs_ ) * modulationB ( _threadargs_ ) ;
   i = g * ( v - e ) ;
   }
 _current += i;
...
{
 { error =  state();
 if(error){fprintf(stderr,"at line 90 in file tmgabaa.mod:\n    SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 }}}
